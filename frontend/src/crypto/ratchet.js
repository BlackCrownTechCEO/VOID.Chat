...
export async function encryptWithSession(session, plaintext) {
  ...
  const encrypted = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, aesKey, utf8ToBytes(plaintext));

  const nextSession = { ...session, sendChainKey: bytesToBase64(chain.nextChainKey), sendCounter: session.sendCounter + 1 };
  return {
    envelope: { ciphertext: bytesToBase64(new Uint8Array(encrypted)), iv: bytesToBase64(iv), counter: nextSession.sendCounter },
    session: nextSession
  };
}
...
export async function decryptWithSession(session, envelope) {
  ...
  const nextSession = { ...session, recvChainKey: bytesToBase64(chain.nextChainKey), recvCounter: session.recvCounter + 1 };
  return { plaintext: bytesToUtf8(new Uint8Array(plaintext)), session: nextSession };
}
