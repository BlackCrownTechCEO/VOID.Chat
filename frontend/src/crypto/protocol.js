import { generateIdentity, exportIdentityBundle, exportIdentityPrivateState, importIdentityPrivateState } from "./keys.js";
import { initiateSession, receiveSession } from "./x3dh.js";
import { encryptWithSession, decryptWithSession } from "./ratchet.js";
...
export async function createEncryptedOutgoing(apiUrl, fromAlias, toAlias, plaintext) {
  ...
  if (!session) {
    ...
    const init = await initiateSession({ identityKeys: identityPrivateState, remoteBundle: remote.bundle });
    session = init.session;
    header = { ...init.header, mode: "prekey" };
  } else {
    header = { mode: "session" };
  }
  ...
  return { header, envelope: encrypted.envelope };
}
